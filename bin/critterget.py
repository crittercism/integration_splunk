...
def setDataRegion(sessionKey):
    """
    :param sessionKey: A string generated by Splunk and used to access its internal API
    :return: None
    """
    use_eu = None
    ...
    global baseurl
    if use_eu:
        if (debug) : print u'{} MessageType="CritterDebug" Using EU data center'.format(DATETIME_OF_RUN)
        baseurl = "https://developers.eu.crittercism.com/v1.0/"
    else:
        if (debug) : print u'{} MessageType="CritterDebug" Using US data center'.format(DATETIME_OF_RUN)
...
def main():
    ...
    # set the data region to either US or EU
